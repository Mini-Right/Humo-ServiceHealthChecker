...
from SHC.plugins.logger.request_id import get_request_id, get_request_sub_id
...
class RequestFilter(logging.Filter):
    def filter(self, record):
        ...
        request_sub_id = get_request_sub_id()
        record.request_id = request_id
        record.request_sub_id = request_sub_id
        return True
...
log_formatter = logging.Formatter('%(asctime)s [%(thread)s] [%(levelname)s] [%(request_id)s] [%(request_sub_id)s] [%(module)s.%(funcName)s] %(message)s')
